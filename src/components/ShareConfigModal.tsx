...
export default function ShareConfigModal({ file, onClose, onCopied }: ShareConfigModalProps) {
  ...
  const getShareURL = () => {
    return `${window.location.origin}/share/file/${file.id}`;
  };
  ...
  const handleGenerateAndCopy = async () => {
    ...
    try {
      await fetch(`/api/files/${file.id}/share-config`, {
        ...
        body: JSON.stringify({ expiryOption, viewOnly }),
      });
      ...
      let msg = labels[expiryOption] || 'Link copied!';
      if (viewOnly) msg += ' (🔒 View-Only Enabled)';
  ...
  return (
    <>
      ...
      >
        <div 
          className="w-full max-w-md bg-surface-container-lowest dark:bg-slate-900 border border-outline-variant/30 dark:border-slate-800 rounded-2xl shadow-2xl p-6 sm:p-8 flex flex-col gap-5"
        ...
        >
              ...
              {[
                ...
                >
                  ...
                  <div className="flex flex-col">
                    ...
                    <span className="text-label-xs font-mono text-outline dark:text-slate-400">{option.desc}</span>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="p-3 bg-surface-container-low rounded-xl border border-outline-variant/30 flex items-center justify-between">
            <div className="flex flex-col">
              ...
              <span className="text-label-xs font-mono text-on-surface-variant">
                Disable raw downloads & text copying
              </span>
            </div>
            <input
              type="checkbox"
          ...
          <div className="grid grid-cols-12 gap-2">
            <button
              ...
              className="col-span-4 h-11 bg-surface-container hover:bg-surface-container-high text-on-surface font-semibold rounded-xl flex items-center justify-center gap-1 shadow-sm transition-all"
            >
            ...
            <button
              ...
              disabled={saving}
              className="col-span-8 h-11 bg-primary hover:bg-primary-container text-on-primary font-semibold rounded-xl flex items-center justify-center gap-2 shadow-sm transition-all active:scale-[0.99]"
            >
              <span className="material-symbols-outlined text-[20px]">
                {saving ? 'progress_activity' : 'content_copy'}
              </span>
              {saving ? 'Configuring...' : 'Copy Link'}
            </button>
          </div>
        </div>
      </div>
